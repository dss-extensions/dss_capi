...
- **This library is not supported by EPRI**. If you find issues or have features specific to the library, please open [an issue ticket on GitHub](https://github.com/dss-extensions/dss_capi/issues/) or send an email (pmeira at ieee.org). For general OpenDSS issues, be sure to test with EPRI's distribution before posting in OpenDSS forum at SourceForge. Users are welcome to post at [our own General Discussion page](https://github.com/orgs/dss-extensions/discussions/categories/general).
...
- There are some new settings:
    ...
    - `Settings_Set_LoadsTerminalCheck`/`Settings_Get_LoadsTerminalCheck`: Use with care! Controls whether the terminals are checked when updating the currents in Load component. Defaults to True (that is, no change from EPRI's OpenDSS). If the loads are guaranteed to have their terminals closed throughout the simulation, this can be set to False to save some precious simulation time.
    - The `DSS_CAPI_EXTENDED_ERRORS` environment variable (also controlled through `Error_Set_ExtendedErrors`): If enabled (default is true since v0.10.6), the active element and active circuit checks use the Error interface to report issues. When disabled, the usual behavior from EPRI's COM implementation is followed -- return a signaling or default value if appropriate.
...
- The DSS command `var` (which lists current DSS language variables) returns all variables in a string in the global result (`Text_Get_Result()`), which is equivalent to EPRI's OpenDSS COM property `DSS.Text.Result`. In COM, this uses a message form (window).
...
- `DynamicExp` objects, `DynamicEq` and `DynOut` properties have been ported in DSS C-API v0.13. 
    - The upstream implementation does not seen mature yet, but we ported already to avoid too much divergency in the features. As with most new features, the implementation is slightly different, especially the code organization, to avoid some potential issues. 
...
- Monitor headers: In EPRI's OpenDSS, since May 2021, the monitor binary stream doesn't include the header anymore. When porting the change to DSS-Extensions, we took the opportunity to rewrite the related code. As such, the implementation in DSS-Extensions deviate from EPRI's one. Extra spaces are not included and should be more consistent. As a recommendation, if your code needs to be compatible with both implementations, trimming the fields should be enough. A compatibility flag exists to try to restore the original behavior.

👉 Besides the omissions and caveats listed here, our general policy on commits/features from EPRI's OpenDSS is to port them as soon as possible. Beside 2025, this used to be done in a matter of hours/days; as of 2025, a more careful analysis is done to avoid porting unstable (new/untested) code, adding tests whenever possible. If we find that a SVN commit/feature in EPRI's OpenDSS introduces issues/bugs, we may postpone porting and/or report the issues in EPRI's OpenDSS forum. Our releases are not coordinated with EPRI's versions.
