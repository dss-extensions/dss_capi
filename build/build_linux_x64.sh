...
if [[ "x${DSS_CAPI_BUILD_CMAKE}" == "x1" ]]; then
    ...
    cmake . ${DSS_EXTENSIONS_EXTRA_CMAKE_FLAGS} -DDSS_EXTENSIONS=ON -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Debug -B build/cmake-debug -DUSE_SYSTEM_EIGEN=OFF -DUSE_SYSTEM_SUITESPARSE=OFF -DBUILD_KLUSOLVEX=OFF -DUSE_SYSTEM_EIGEN=OFF -DUSE_SYSTEM_SUITESPARSE=OFF -DBUILD_KLUSOLVEX=OFF
