...
begin
    ...
    if DSS.ActiveCircuit.AutoAddBusList.ListSize > 0 then
        FBusList := DSS.ActiveCircuit.AutoAddBusList
    else

    if DSS.ActiveCircuit.EnergyMeters.ListSize = 0 then
    begin
        // No energymeters in circuit
        // Include all buses in the circuit
        BusIdxListSize := DSS.ActiveCircuit.BusList.listsize;
    ...
    begin
        ...
        FBusList := THashList.Create(DSS.ActiveCircuit.NumBuses);
    ...
    for i := 1 to BusIdxListSize do
    begin
        BusIdxList^[i] := DSS.ActiveCircuit.BusList.Find(FbusList.Get(i));
    end;
...
{  Algorithm:
    ...
    {Start up Log File}
        ...
        AssignFile(FLog, DSS.OutputDirectory + DSS.CircuitName_ + 'AutoAddLog.CSV');
        Rewrite(FLog);
    ...
    {Turn regulators and caps off while we are searching}
        ...
        case AddType of
            ...
            begin
                ...
                DSS.GlobalResult := BusList.Get(MinLossBus) +
