  ...
  WITH ActiveStorageControllerObj Do Begin
     ...
     WHILE Length(Param)>0 Do Begin
         ...
         CASE ParamPointer OF
            ...
            propKWTARGETLOW: FkWTargetLow  := Parser.DblValue;
            propKWBAND:   FpctkWBand       := Parser.DblValue;
            propKWBANDLOW:   FpctkWBandLow := Parser.DblValue;
       ...
       Begin
            ...
            Begin
               ...
               If abs(PDiff) > HalfkWBandLow Then
                 Begin // Attempt to change storage kW charge
                       ...
                       If ShowEventLog Then  AppendToEventLog('StorageController.' + Self.Name, Format('Attempting to charge %-.6g kW with %-.6g kWh remaining and %-.6g rating.', [PDiff, (TotalRatingkWh-ActualkWh), TotalRatingkWh]));
                       For i := 1 to FleetSize Do
                       Begin
                            StorageObj := FleetPointerList.Get(i);
                            WITH StorageObj Do
            ...
            Else
              Begin If not FleetState = STORE_IDLING Then
                    ...
                    End;
                    ChargingAllowed := FALSE;
                    If ShowEventLog Then  AppendToEventLog('StorageController.' + Self.Name, Format('Fully charged: %-.6g kWh of rated %-.6g.', [ActualkWh, TotalRatingkWh]));
              End;
       End;
       ...
       ChargingAllowed := FALSE;
{
  Check discharge mode first. Then if not discharging, we can check for charging
       ...
       CASE ChargeMode of
            ...
            MODEPEAKSHAVELOW: DoPeakShaveModeLow
       ELSE
