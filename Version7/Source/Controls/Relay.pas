   ...
   {See if we can find this Relay name in the present collection}
    ...
    if OtherRelay <> NIL then
        ...
        begin
            ...
            if DebugTrace then
                AppendToEventLog('Relay.' + self.Name, Format('MakeLike NumReclose=%d', [NumReclose]));
    ...
    begin
        ...
        case Code of
            ...
            Integer(CTRL_RESET):
                ...
                begin
                    if ShowEventLog then
                        AppendToEventLog('Relay.' + Self.Name, 'Reset');
                    Reset;
                    if ControlType = td21 then
                        td21_quiet := td21_pt div 2
                end
...
begin
    ...
    if not LockedOut then
        ...
        begin
            ...
            if DebugTrace then
                AppendToEventLog('Relay.' + self.Name, Format('FaultDetected=%s', [BoolToStr(FaultDetected)]));
            MonitoredElement.GetTermVoltages(MonitoredElementTerminal, cvBuffer);
            if td21_i < 1 then
            begin
                if DebugTrace then
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            ...
                            if DebugTrace then
                                AppendToEventLog('Relay.' + self.Name, Format('Zhsd[%d,%d]=%.4f+j%.4f, Zdir=%.4f+j%.4f', [i, j, Zhsd.re, Zhsd.im, Zdir.re, Zdir.im]));
                            if (Zdir.re > 0.0) and (Zdir.im > 0.0) then
                            begin
                                Uhsd := csub(cmul(Zhsd, Iloop), Vloop);
                                Uhsd2 := cabs2(Uhsd);
                                if DebugTrace then
                                    AppendToEventLog('Relay.' + self.Name, Format('     Uhsd=%.2f, Uref=%.2f', [cabs(Uhsd), cabs(Uref)]));
                    ...
                    if not ArmedForOpen then
                        ...
                        begin
                            ...
                            begin
                                LastEventHandle := ControlQueue.Push(Solution.DynaVars.intHour, t_event + RecloseIntervals^[OperationCount], CTRL_CLOSE, 0, Self);
                                if DebugTrace then
                                    AppendToEventLog('Relay.' + self.Name, Format('Pushing reclose event for %.3f', [t_event + RecloseIntervals^[OperationCount]]));
                                ArmedForClose := TRUE;
                            end;
                        end;
                    Targets.Free();
                ...
                begin  // not picked up; reset if necessary
                    ...
                    begin // this implements the reset, whether picked up or not
                        ...
                        with ActiveCircuit do
                            LastEventHandle := ControlQueue.Push(Solution.DynaVars.intHour, Solution.DynaVars.t + ResetTime, CTRL_RESET, 0, Self);
                        if DebugTrace then
                            AppendToEventLog('Relay.' + self.Name, Format('Pushing reset event for %.3f', [ActiveCircuit.Solution.DynaVars.t + ResetTime]));
                    end;
                    if ArmedForOpen then
                    begin
                        td21_quiet := td21_pt + 1;
