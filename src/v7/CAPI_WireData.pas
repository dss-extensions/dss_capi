...
begin
    ...
    if ActiveCircuit <> NIL then
        with ActiveCircuit do
            if WireDataClass.ElementList.ListSize > 0 then
            begin
                DSS_RecreateArray_PPAnsiChar(Result, ResultPtr, ResultCount, (WireDataClass.ElementList.ListSize - 1) + 1);
                k := 0;
                WireDataElem := WireDataClass.ElementList.First;
                while WireDataElem <> NIL do
                begin
                    Result[k] := DSS_CopyStringAsPChar(WireDataElem.Name);
                    Inc(k);
                    WireDataElem := WireDataClass.ElementList.Next;
                end;
            end;
