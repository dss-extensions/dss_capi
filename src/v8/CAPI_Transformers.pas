...
begin
    ...
    if ActiveCircuit[ActiveActor] = NIL then
        Exit;
    Generic_Get_AllNames(ResultPtr, ResultCount, ActiveCircuit[ActiveActor].Transformers, False);
end;
//------------------------------------------------------------------------------
function Transformers_Get_First(): Integer; CDECL;
var
...
begin
    ...
    elem.GetLosses(CResult[1], CResult[2], CResult[3], ActiveActor);
...
var
    ...
    k: Integer;
begin
    if (ActiveCircuit[ActiveActor] = NIL) or (ActiveCircuit[ActiveActor].Transformers.ListSize <= 0) then
    begin
        Result := DSS_RecreateArray_PDouble(ResultPtr, ResultCount, 1);
        Result[0] := 0;
        Exit;
    ...
    while elem <> NIL do
    begin
        elem.GetLosses(CResult[k], CResult[k + 1], CResult[k + 2], ActiveActor);
        elem := lst.Next;
        Inc(k, 3);
    end;
