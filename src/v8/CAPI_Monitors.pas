...
begin
    ...
    if ActiveCircuit[ActiveActor] = NIL then
        Exit;
    Generic_Get_AllNames(ResultPtr, ResultCount, ActiveCircuit[ActiveActor].Monitors, False);
end;
...
begin
    ...
    if ActiveCircuit[ActiveActor] = NIL then
        Exit;
    if ActiveCircuit[ActiveActor].Monitors.Active = NIL then
        Exit;
    ReadMonitorHeader(Header, TRUE);
    ...
    begin
        ...
        while k < ListSize do
        begin
            Result[k] := DSS_CopyStringAsPChar(AuxParser[ActiveActor].StrValue);
            Inc(k);
        end;
