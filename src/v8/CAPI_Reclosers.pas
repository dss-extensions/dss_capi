...
begin
    ...
    cmd := Format('recloser.%s.%s=%s', [TRecloserObj(RecloserClass[ActiveActor].GetActiveObj).Name, parm, val]);
...
begin
    ...
    begin
        if RecloserClass[ActiveActor].ElementList.ListSize > 0 then
        begin
            pList := RecloserClass[ActiveActor].ElementList;
...
begin
    Result := 0;
    if ActiveCircuit[ActiveActor] <> NIL then
        Result := RecloserClass[ActiveActor].ElementList.ListSize;
end;
//------------------------------------------------------------------------------
function Reclosers_Get_First(): Integer; CDECL;
var
...
begin
    ...
    if ActiveCircuit[ActiveActor] <> NIL then
    begin
        pElem := RecloserClass[ActiveActor].ElementList.First;
        if pElem <> NIL then
            repeat
                if pElem.Enabled then
                begin
                    ActiveCircuit[ActiveActor].ActiveCktElement := pElem;
                    Result := RecloserClass[ActiveActor].ElementList.ActiveIndex;
                end
                else
                    pElem := RecloserClass[ActiveActor].ElementList.Next;
...
begin
    Result := '';
    elem := RecloserClass[ActiveActor].GetActiveObj;
    if elem <> NIL then
        Result := elem.Name;
end;
...
begin
    if ActiveCircuit[ActiveActor] <> NIL then
    begin
        if RecloserClass[ActiveActor].SetActive(Value) then
        begin
            ActiveCircuit[ActiveActor].ActiveCktElement := RecloserClass[ActiveActor].ElementList.Active;
        end
        else
        begin
...
begin
    Result := '';
    elem := RecloserClass[ActiveActor].ElementList.Active;
    if elem <> NIL then
...
begin
    if ActiveCircuit[ActiveActor] <> NIL then
    begin
        pRecloser := RecloserClass[ActiveActor].Elementlist.Get(Value);
