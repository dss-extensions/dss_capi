...
type
    ...
    TBusHashListType = TAltHashList;
    TCommandHashListType = TAltHashList;
    TClassNamesHashListType = TAltHashList;
...
procedure THashList.DumpToFile(F: TStream);
var
    i, j: Integer;
    sout: String;
begin
...
function TAltHashList.Add(const S: String; Value: Integer): Integer; inline;
begin
    if Value < 0 then
        Value := self.Count + 1;
...
begin
    ...
    for i := 1 to Count do
    begin
        FSWriteln(F, Format('%3d = "%s"', [i, NameOfIndex(i)]));
    end;
end;
