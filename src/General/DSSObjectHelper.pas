...
begin
    ...
    case ptype of
        ...
        begin
            SetObjInteger(Obj, Index, TDSSEnum(Pointer(PropertyOffset2[Index])).StringToOrdinal(LowerCase(Value)), @prevInt);
            Result := True;
        end;
        ...
        TPropertyType.MappedStringEnumProperty:
            begin
            if (TPropertyFlag.ConditionalReadOnly in flags) and (PBoolean(PByte(obj) + PropertyOffset3[Index])^) then
            begin
                Result := True;
                Exit;
            end;
        ...
        begin
            if TPropertyFlag.ScaledByFunction in flags then
                scale := TDoublePropertyFunction(Pointer(PropertyOffset2[Index]))(obj)
            else
                scale := PropertyScale[Index];
        ...
        begin
            ...
            begin
                ElemName := ConstructElemName(DSS, lowercase(Value));  // substitute @var value if any
        ...
        begin
            ...
            begin
                ...
                begin
                    while Length(PropParser.StrValue) > 0 do
                    begin
                        ElemName := ConstructElemName(DSS, lowercase(PropParser.StrValue));
                        intVal := GetCktElementIndex(DSS, ElemName);
                        otherObj := NIL;
                        if intVal > 0 then
                            otherObj := DSS.ActiveCircuit.CktElements.Get(intVal);
...
begin
    ...
    begin
        case PropertyType[Index] of
            ...
            TPropertyType.DoubleOnArrayProperty,
            TPropertyType.DoubleOnStructArrayProperty:
                PropStr := FloatToStr(GetObjDouble(obj, Index));
            ...
            begin
                ...
                begin
                    ...
                    ValueCount[1] := 0;
                    otherObjPtr0 := NIL;
                    TObjRefsPropertyFunction(Pointer(PropertyWriteFunction[Index]))(obj, PPointer(otherObjPtr0), @ValueCount[0]);
...
begin
    ...
    begin
        ...
        begin
            boolPtr := PBoolean(PByte(obj) + PropertyOffset[Index]);
            if prevInt <> NIL then
    ...
    case ptype of
        ...
        TPropertyType.MappedStringEnumProperty:
        begin
            if TPropertyFlag.IntegerOnArray in flags then
            begin
                integerPtr := PPInteger(PByte(obj) + PropertyOffset[Index])^;
...
var
    i, j, maxSize, Norder, intVal: Integer;
...
begin
    ...
    case PropertyType[Index] of 
        ...
        begin
            // Number of items
            maxSize := PInteger(PByte(obj) + PropertyOffset[Index])^;
            // Current position
            positionPtr := PInteger(PByte(obj) + PropertyStructArrayIndexOffset);
...
begin
    ...
    case PropertyType[Index] of
        ...
        TPropertyType.MappedStringEnumProperty:
            Result := TDSSEnum(Pointer(PropertyOffset2[Index])).OrdinalToString(
                PInteger(PByte(obj) + PropertyOffset[Index])^
            );
...
begin
    ...
    case PropertyType[Index] of
        ...
        begin
            if (TPropertyFlag.ReadByFunction in PropertyFlags[Index]) then
                Result := TIntegerPropertyFunction(Pointer(PropertyOffset2[Index]))(obj)
            else
        ...
        TPropertyType.BooleanProperty:
            Result := Integer(PBoolean(PByte(obj) + PropertyOffset[Index])^);
...
begin
    ...
    case ptype of
        ...
        begin
            // Number of items
            count := PInteger(PByte(obj) + PropertyOffset[Index])^;
            if count <= 0 then
                Exit;
...
begin
    ...
    case PropertyType[Index] of
        ...
        begin
            if TPropertyFlag.ReadByFunction in PropertyFlags[Index] then
            begin
                TObjRefsPropertyFunction(Pointer(PropertyWriteFunction[Index]))(obj, ResultPtr, ResultCount);
                Exit;
            end;
