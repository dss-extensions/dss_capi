    ...
    Begin
      ...
      Begin
        ...
        Begin
          ...
          Begin
            PMult := TLoadshapeObj(ActiveDSSObject).GetPMult(iElem);
            myLoadShape[iElem]  :=  myLoadShape[iElem]  + PMult;
            QMult := TLoadshapeObj(ActiveDSSObject).GetQMult(iElem);
            if not IsNaN(QMult) then
            Begin
              myWeight    :=  QMult;
              if myWeight = 0 then
              Begin
                ...
                Begin  // user specified the PF for this load
                  myWeight  := PMult * SQRT((1.0/SQR(myPF) - 1));
        ...
        myLoadShapes[High(myLoadShapes)]    :=  GetCurrentDir + PathDelim + 'loadShape_' + EMeter.Name + '.csv';
