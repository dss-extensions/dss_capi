...
begin
    ...
    begin
        inStream := TFilestream.Create(inFn, fmOpenRead or fmShareDenyWrite);
        try
...
begin
    ...
    myFile := TFileStream.Create(Path + PathDelim + 'Master.dss', fmOpenRead or fmShareDenyWrite);
...
begin
    Result := FALSE;
    try
        F := TFileStream.Create(DSS.OutputDirectory {DSS.CurrentDSSDir} + 'BusVoltageBases.dss', fmCreate);
...
begin
    Result := FALSE;
    try
        F := TFileStream.Create(DSS.OutputDirectory {DSS.CurrentDSSDir} + 'Master.dss', fmCreate);
        FSWriteln(F, 'Clear');
...
begin
    ...
    try
        F := TFileStream.Create(DSS.OutputDirectory {DSS.CurrentDSSDir} + 'BusCoords.dss', fmCreate);
