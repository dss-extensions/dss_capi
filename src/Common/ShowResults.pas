...
begin
  ...
  {Get Bus Reference}
    ...
    begin
        DoSimpleMsg(DSS, 'Bus "%s" not found.', [UpperCase(BusName)], 219);
        Exit;
    end;
    try
        F := TBufferedFileStream.Create(FileNm, fmCreate);
                ...
                for iBus := 1 to NumBuses do
                    ...
                    begin
                        ...
                        begin
                            ...
                            WriteStr(sout, Pad(EncloseQuotes(UpperCase(BusList.NameOfIndex(iBus))), MaxBusNameLength + 2), GetNum(Iphs): 4, GetNum(Iphs + 1): 4, Cabs((VFault^[iphs] - VFault^[iphs + 1]) * GFault): 12: 0, '   ');
                            FSWrite(F, sout);
                            for i := 1 to NumNodesThisBus do
                            begin
                                Vphs := Cabs(VFault^[i]);
