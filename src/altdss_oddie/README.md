...
Since Oddie tries to work around some limitations and provide minor complements to the APIs, there are a few extra error messages.

If an error message contains **`(Oddie)`**, it is generated by Oddie itself.

Regarding how to report more general issues, a recommendation would be to check if the issue is present in other official OpenDSS alternatives, such as the GUI or the COM API implementation. If it can be reproduced that way, the recommended place to report is at https://sourceforge.net/p/electricdss/discussion/ 

Otherwise, if the issue is only present when running Oddie, feel free to report on DSS-Extensions. It is possible that the issue is Oddie, or there is an issue in the official OpenDSSDirect.DLL. Depending on the details of the issue, it might be hard for users to differentiate. This is a new project, so let's see how it goes.

When in doubt, users are welcome to create a GitHub issue or ask question in the Discussions page on DSS-Extensions. Please provide some basic information, e.g.:
- OS version
- The hosting executable/package/language (e.g. "using DSS-Python"),
- How to reproduce the error or suspicious behavior; it really helps us if you have a simple .DSS script, Python/Julia/MATLAB script, or can write one based on the sample circuits distributed with OpenDSS (or at our (electricdss-tst)(https://github.com/dss-extensions/electricdss-tst) repository).

See also http://www.sscce.org/ and https://stackoverflow.com/help/minimal-reproducible-example for other recommendations on how to report problems.
...
made on AltDSS was achieved using the official (EPRI's) OpenDSS library.
...
In July 2024, there were a few bugfixes and the new functions for the Storages and WindGens APIs were implemented.
...
the same engine will be wrapped multiple times, avoid that!
