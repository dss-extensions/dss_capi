...
def skip(fun):
    prefixes = [
        ...
        'DSS_BeginPascalThread', 'DSS_SetMessagesMO', 'DSS_SetPropertiesMO', 'Alt_', 'ctx_ShareGeneral', 'AltDSSCAPIInit']
    for p in prefixes:
        if p in fun:
            return True
...
for fn in glob('src/CAPI/*.pas'):
    ...
    with open(fn, 'r') as fi, open('build/generated/' + bnctx, 'w') as fo:
        ...
        src = src.replace('DSSPRIME', 'DSSPrime')
        src = src.replace('(DSS, DSS, ', '(DSS, ')
        src = src.replace('(DSS, DSS)', '(DSS)')
...
with open('include/altdss/capi/dss.h', 'r') as fi:
    header = fi.read()
    
with open('include/altdss/capi/dss_ctx.h', 'w') as fo:    
    fo.write('/*\n')
    fo.write('    REMINDER:\n')
    fo.write('    This file is automatically generated from altdss/capi/dss.h\n')
    fo.write('    by the classic_to_ctx.py script. DO NOT EDIT MANUALLY!\n')
    fo.write('    If you need to edit this, remember to disable running\n')
    fo.write('    classic_to_ctx.py in the build scripts.\n')
    fo.write('*/\n\n')
    ...
    functions = [
        re.sub(r'(    ALTDSS_CAPI_DLL [^ ]+[ \*]*)([a-zA-Z0-9_]+)\(', r'\1ctx_\2(const void* ctx, ', function) 
...
extern "C" {
    ...
    */
    ...
    by the classic API.
    */
    ALTDSS_CAPI_DLL const void* ctx_Get_Prime(void);
    ...
    */
    ALTDSS_CAPI_DLL const void* ctx_Set_Prime(const void *ctx);
