...
if exist ..\electricdss-src\Source\Common\DSSGlobals.pas (
    ...
    fpc -Pi386 @src\v8\windows-x86.cfg -B src\v8\dsspm_capi.lpr
    if exist lib\win_x86\dsspm_capi.dll (
        where /q dumpbin
        if errorlevel 1 (
            echo WARNING: dumpbin.exe is not in your path. Be sure to run this script on 
            echo          the "x86 Native Tools Command Prompt for VS 2017" or the 
        ...
        dumpbin /exports "lib\win_x86\dsspm_capi.dll" > lib\win_x86\exports.txt
        echo LIBRARY DSSPM_CAPI > lib\win_x86\dsspm_capi.def
        echo EXPORTS >> lib\win_x86\dsspm_capi.def
        for /f "skip=19 tokens=4" %%A in (lib\win_x86\exports.txt) do echo %%A >> lib\win_x86\dsspm_capi.def
        lib /def:lib\win_x86\dsspm_capi.def /out:lib\win_x86\dsspm_capi.lib /machine:X86
        del /s lib\win_x86\dsspm_capi.exp
        del /s lib\win_x86\dsspm_capi.def
        del /s lib\win_x86\exports.txt
