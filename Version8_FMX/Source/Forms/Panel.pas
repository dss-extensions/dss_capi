...
uses
  ...
  Vcl.ExtCtrls, Vcl.Controls, FMX.ImgList,
...
type
  TControlPanel = class(TForm)
    ...
    ToolButton11: TButton;
    ClassBox: TComboBox;
    ElementBox: TComboBox;
    ToolButton12: TButton;
    ToolButton13: TButton;
    ...
    Growth1: TMenuItem;
    Year1: TMenuItem;
    ToolButton14: TButton;
    ToolButton15: TButton;
    ...
    ToolButton21: TButton;
    ToolButton23: TButton;
    ToolButton24: TButton;
    ToolButton25: TButton;
    CurrentsElemResid1: TMenuItem;
    RPNEvaluator1: TMenuItem;
    Yprims1: TMenuItem;
    Y1: TMenuItem;
    ToolButton20: TButton;
    ...
    Panel5: TPanel;
    ResultPages: TTabControl;
    EditPages: TTabControl;
    SummaryTab: TTabItem;
    ResultsTab: TTabItem;
    Label1: TLabel;
...
uses Executive, DSSClassDefs, DSSGlobals,
  ...
  IniRegSave, Solution, ControlQueue;
...
Begin
//DavisMMP
{  Case MessageDlg('File '+ActiveScriptForm.Text+' has changed.  Save ?', TMsgDlgType.mtConfirmation, [TMsgDlgBtn.mbYes, TMsgDlgBtn.mbNo], 0) of
...
{  If ActiveScriptForm.HasFileName Then Begin
    DSS_Registry.WriteString(Format ('File%d', [idx]), ActiveScriptForm.Text);
    Result := TRUE;
  End;}
End;
...
begin
  ...
  if Length(FileName) > 0 then begin
    Try
      ...
      //DavisMMP
      //      ActiveScriptForm.Text  := ExpandFileName(FileName);
...
//DavisMMP
//  If (Self.Position.X > Screen_Width) or (Self.Position.X < 0) then Self.Position.X := 0;
...
//     StatusBar1.Panels[1].Text := Format('Blocks: %d',[AllocMemCount]);
  ...
  DemandInterval1.IsChecked    := EnergyMeterclass[ActiveActor].SaveDemandInterval[ActiveActor] ;
  Caption := ProgramName + ' Data Directory: ' + DataDirectory[ActiveActor]; // NOTE: not necessarily same as output directory
  If ActiveCircuit[ActiveActor] <> nil then
    With ActiveCircuit[ActiveActor] Do Begin
      ZonesLocked1.IsChecked        := ZonesLocked;
      DuplicatesAllowed1.IsChecked  := DuplicatesAllowed;
      TraceLog1.IsChecked           := ControlQueue.TraceLog;
      Trapezoidal1.IsChecked        := TrapezoidalIntegration;
    End;
//DavisMMP
//  LBL_DefaultFreq.Text  := Format(' Base Frequency = %d Hz', [Round(DefaultBaseFreq) ]);
  UpdateCaptions;
end;

procedure TControlPanel.RecordScript1Click(Sender: TObject);
begin
  RecordScript1.IsChecked  := NOT  RecordScript1.IsChecked ;
  RecordCommands        := RecordScript1.IsChecked;
end;
...
begin
       ...
       Begin
         With PlotOptionsForm Do Begin
            ...
            //DavisMMP
            //            If LoopCheck.IsChecked  Then S := S + ' showloops=y';
...
{  MessageEdit.Clear;
  if ActiveScriptForm.HasFileName then begin
    ...
    MessageEdit.Lines.Add(ProgramName + ' - ' + ActiveScriptForm.Text);
    ActiveScriptForm.Tab.Text  := ExtractFileName(ActiveScriptForm.Text );
  end else begin
    MessageEdit.Lines.Add(ActiveScriptForm.Text);
  end;
...
begin
     DemandInterval1.IsChecked  := NOT DemandInterval1.IsChecked ;
     EnergyMeterclass[ActiveActor].SaveDemandInterval[ActiveActor]:= DemandInterval1.IsChecked;
end;
...
begin
     If ActiveCircuit[ActiveActor] <> Nil then
     With ActiveCircuit[ActiveActor] Do Begin
       ZonesLocked1.IsChecked  := NOT ZonesLocked1.IsChecked ;
       ZonesLocked := ZonesLocked1.IsChecked;
     End;
end;
...
begin
     If ActiveCircuit[ActiveActor] <> Nil then
     With ActiveCircuit[ActiveActor] Do Begin
       DuplicatesAllowed1.IsChecked  := NOT DuplicatesAllowed1.IsChecked ;
       DuplicatesAllowed := DuplicatesAllowed1.IsChecked;
     End;
end;
...
begin
     If ActiveCircuit[ActiveActor] <> Nil then
     With ActiveCircuit[ActiveActor] Do Begin
       TraceLog1.IsChecked  := NOT TraceLog1.IsChecked ;
       ControlQueue.TraceLog := TraceLog1.IsChecked;
     End;
...
begin
     If ActiveCircuit[ActiveActor] <> Nil then
     With ActiveCircuit[ActiveActor] Do Begin
       Trapezoidal1.IsChecked  := NOT Trapezoidal1.IsChecked ;
       TrapezoidalIntegration := Trapezoidal1.IsChecked;
     End;
end;
...
//        Options := [ofOverwritePrompt];
        If Execute Then Begin
//           ActiveScriptForm.Text  := FileName;
