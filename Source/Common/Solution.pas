...
begin
    ...
    begin
        ...
        Update_dblHour;
  {$IFDEF FPC}
        if Assigned(ActiveFNCS) then
        begin
            if ActiveFNCS.IsReady then
            begin
...
//        writeln('Loop time is ' + format('%d', [intHour]) + ':' + format('%d', [Trunc(t)]));
        ...
        if Assigned(ActiveHELICS) then
        begin
            if ActiveHELICS.IsRunning then
            begin
            ...
            begin
              ...
              // Normal solution routine
                ...
                // Sends a message to Actor Object (UI) to notify that the actor has finised
                    ...
                    SOLVE_AD1:
                        SolveAD(ActorID, TRUE);               // Solves the model if the actor has PIE
                    SOLVE_AD2:
                        SolveAD(ActorID, FALSE);              // Complements the solution
                    ZEROIVECTOR:
                        ZeroInjCurr(ActorID);
                    GETCURRINJ:
                        GetSourceInjCurrents(ActorID);
