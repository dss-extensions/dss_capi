...
var
    i: Integer;
begin
    if pAuto.NumberOfWindings > nWindings then
        nWindings := pAuto.NumberOfWindings;
    bAuto := TRUE;
    pd_unit := pAuto;
    for i := 1 to pAuto.NumberOfWindings do
    begin
        phaseA[i - 1] := 1;
        phaseB[i - 1] := 1;
        phaseC[i - 1] := 1;
...
//        if FD.Separate then EndInstance (SshPrf, 'PowerElectronicsConnection');
        ...
        with ActiveCircuit[ActiveActor] do
        begin
            if (InvControls2.ListSize > 0) or (ExpControls.ListSize > 0) then
            begin
    ...
    // voltage regulators
        ...
        begin
            ...
            begin
                ...
                if UseLimit then
                begin // maxLimitVoltage only in OpenDSS
